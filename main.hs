...
cameraShader diffuseColor hitpoint surf scn = map ( round . ( (*) ratio) . fromIntegral  ) $ diffuseColor
				where
...
cameraLinFalloffShader diffuseColor luminosity hitpoint surf scn = map ( round . ( (*) ratio) . fromIntegral  ) $ diffuseColor
				where
...
scene = [ 
		  (Surface Absorb (Vector [0.7, 0.7, 1]) (sphereHit 0.25) (cameraShader [0, 165, 255] )),
		  (Surface Absorb (Vector [0.3, 0.3, 2]) (sphereHit 0.25) (cameraShader [255, 165, 0] ))
		  ]
-- Image info
width = 800
height = 800
